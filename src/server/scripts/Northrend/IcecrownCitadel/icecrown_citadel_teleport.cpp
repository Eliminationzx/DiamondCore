    ...
    public:
        ...
        bool OnGossipHello(Player* player, GameObject* go)
        {
            if (go->GetEntry() != GO_SCOURGE_TRANSPORTER_FIRST)
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Телепорт в Молот Света.", GOSSIP_SENDER_ICC_PORT, LIGHT_S_HAMMER_TELEPORT); // M_PI + M_PI/6
            if (InstanceScript* instance = go->GetInstanceScript())
            {
                if (instance->GetBossState(DATA_LORD_MARROWGAR) == DONE && go->GetEntry() != 202245)
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Телепорт в Молельня проклятых.", GOSSIP_SENDER_ICC_PORT, ORATORY_OF_THE_DAMNED_TELEPORT); // M_PI + M_PI/6
                if (instance->GetBossState(DATA_LADY_DEATHWHISPER) == DONE && go->GetEntry() != 202243)
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Телепорт в Рампарт Черепов.", GOSSIP_SENDER_ICC_PORT, RAMPART_OF_SKULLS_TELEPORT); // M_PI/6
                if (instance->GetBossState(DATA_ICECROWN_GUNSHIP_BATTLE) == DONE && go->GetEntry() != 202244)
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Телепорт в Взлет Смертоносного.", GOSSIP_SENDER_ICC_PORT, DEATHBRINGER_S_RISE_TELEPORT); // M_PI/6
                if (instance->GetData(DATA_COLDFLAME_JETS) == DONE && go->GetEntry() != 202235)
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Телепорт в Телепорт в Верхней шпиль.", GOSSIP_SENDER_ICC_PORT, UPPER_SPIRE_TELEPORT); // M_PI/6
				if (instance->GetBossState(DATA_VALITHRIA_DREAMWALKER) == DONE && instance->GetBossState(DATA_SINDRAGOSA_GAUNTLET) == DONE && go->GetEntry() != 202246)
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Телепорт в логово Синдрагосы.", GOSSIP_SENDER_ICC_PORT, SINDRAGOSA_S_LAIR_TELEPORT); // M_PI*3/2 + M_PI/6
            }
