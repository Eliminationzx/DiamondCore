...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        if (sT->GetEnableTransmogInfo())
            //player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Misc_Book_11:30:30:-18:0|tDeveloper transmogrification", EQUIPMENT_SLOT_END+9, 0);
...
#endif
        player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Enchant_Disenchant:30:30:-18:0|tУдалить все трансмогрификации", EQUIPMENT_SLOT_END+2, 0, "Удалить трансмогрификации из всех надетых предметов?", 0, false);
        ...
        case EQUIPMENT_SLOT_END+6: // view preset
            {
                ...
                player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Misc_Statue_02:30:30:-18:0|tИспользуйте набор", EQUIPMENT_SLOT_END+5, action, "Используя этот выбор вещи будут превращены и их нельзя будет вернуть продовцу. \nВы хотите продолжить?\n\n"+sT->presetByName[player->GetGUID()][action], 0, false);
                player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "|TInterface/PaperDollInfoFrame/UI-GearManager-LeaveItem-Opaque:30:30:-18:0|tУдалить сет", EQUIPMENT_SLOT_END+7, action, "Вы уверены, что хотите удалить "+sT->presetByName[player->GetGUID()][action]+"?", 0, false);
        ...
        case EQUIPMENT_SLOT_END+8: // Save preset
            {
                ...
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, "|TInterface/PaperDollInfoFrame/UI-GearManager-Undo:30:30:-18:0|Обновить меню", sender, action);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/Ability_Spy:30:30:-18:0|tНазад..", EQUIPMENT_SLOT_END+4, 0);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            } break;
         ...
         if (oldItem)
         {
             ...
             for (uint8 i = INVENTORY_SLOT_ITEM_START; i < INVENTORY_SLOT_ITEM_END; ++i)
             {
                 ...
                 if (sT->GetFakeEntry(oldItem->GetGUID()) == newItem->GetEntry())
                     continue;
                 ++limit;
                 player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, sT->GetItemIcon(newItem->GetEntry(), 30, 30, -18, 0)+sT->GetItemLink(newItem, session), slot, newItem->GetGUIDLow(), "Используя этот выбор вещи будут превращены и их нельзя будет вернуть продовцу. \nВы хотите продолжить?\n\n"+sT->GetItemIcon(newItem->GetEntry(), 40, 40, -15, -10)+sT->GetItemLink(newItem, session)+ss.str(), price, false);
             }
