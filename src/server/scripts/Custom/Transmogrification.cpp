...
const char * Transmogrification::GetSlotName(uint8 slot, WorldSession* session) const
{
    ...
    case EQUIPMENT_SLOT_MAINHAND  : return  "Основная рука";// session->GetTrinityString(LANG_SLOT_NAME_MAINHAND);
    ...
    case EQUIPMENT_SLOT_TABARD    : return  "Гильдейская накидка";// session->GetTrinityString(LANG_SLOT_NAME_TABARD);
    default: return NULL;
    }
}
